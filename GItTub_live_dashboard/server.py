...
PARTIAL_INTERVAL_BYTES = 16000   # roughly 0.5 sec at 16 kHz, 16-bit mono
SAMPLE_RATE = 16000
CHANNELS = 1
SAMPLE_WIDTH = 2
...
app = FastAPI(title="GItTub Backend")
...
def transcribe_audio_sync(pcm_audio: bytes) -> str:
    """Convert raw 16 kHz / 16-bit / mono PCM to WAV and transcribe."""
    if not pcm_audio:
        return ""
    ...
    try:
        while True:
            ...
            if "bytes" in message and message["bytes"] is not None:
                ...
                # Send cumulative partial transcription while audio is coming.
                if (
                    is_verified
                    and len(session_audio_buffer) - last_partial_size >= PARTIAL_INTERVAL_BYTES
                ):
                    partial_text = await asyncio.to_thread(
                        transcribe_audio_sync,
                        bytes(session_audio_buffer)
                    )
                    last_partial_size = len(session_audio_buffer)
